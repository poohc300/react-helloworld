...
# Routing
Routing이란 URL에 따라서 그에 상응하는 화면을 전송해주는 것이다. 리액트에서 React Router를 사용하지 않았을 경우 다음과 같은 문제점이 발생한다
## react-router-dom 사용하지 않을 떄 문제점
특정 메뉴 버튼을 클릭해서 그에 상응하는 컴포넌트로 상태가 바뀌는 방식은 사용자 입장에서 문제점이 있다.
...
- SEO 측면에서도 일반 웹사이트들과 차이가 있어서 원치않는 방식으로 색인이 될 수 있다.
## React Router
리액트에서 페이지를 이동할 때 필요한 라이브러리다. 원래는 A 페이지를 보여주고 싶다면 A.html 파일을 이용하고 B페이지를 보여주고 싶다면 B.html 파일을 보여주는 방식이었다. 하지만 리액트에서는 웹 사이트의 전체 페이지를 하나의 페이지에 담아 동적으로 화면을 바꿔가며 표현한다. 이를 SPA라고 한다.  
### SPA 에서의 페이지 전환(브라우징)
HTML5 의 History API를 사용해서 가능하게 만든다. 자바스크립트 영역에서 History API를 이용해 현재 페이지 내에서 페이지 이동이 일어난 것 처럼 보여준다.  https://v5.reactrouter.com/web/guides/quick-start